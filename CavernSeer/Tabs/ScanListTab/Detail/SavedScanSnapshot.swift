...
struct SavedScanSnapshot: View {
    ...
    var body: some View {
        HStack {
            self.showSnapshot(snapshot: scan?.startSnapshot) .map { styleAndAddContextMenu(uiImg: $0, suffix: "start") }
            
            Spacer().frame(width: 30)
            
            self.showSnapshot(snapshot: scan?.endSnapshot) .map { styleAndAddContextMenu(uiImg: $0, suffix: "end") }
