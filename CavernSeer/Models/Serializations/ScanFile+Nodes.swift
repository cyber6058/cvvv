...
extension ScanFile {
    func toSCNNodes(
        ...
        let lineNodes_1 = self.lines.map {
            line in
            line.toSCNNode(stationDict: stationDict, lengthPref: lengthPref)
        }
        
        let lineNodes_2 = self.lines.map {
            line in
            line.toSCNNode_thick(stationDict: stationDict, lengthPref: lengthPref)
        }

        return meshAnchorNodes + lineNodes_1 + lineNodes_2 + stationNodes
    }
}
