...
final class ScanFile : NSObject, NSSecureCoding, StoredFileProtocol {
    ...
    convenience init(
        ...
        self.init(
            ...
            stations: stations.map { SurveyStation(entity: $0, name: $0.bbox_loc) },
            lines: lines.map { SurveyLine(entity: $0, name: $0.linked_bbox_loc) },
    ...
    required init?(coder: NSCoder) {
        ...
        do {
            ...
            self.startSnapshot = try decoder.snapshot(key: PropertyKeys.startSnapshot)
            self.endSnapshot = try decoder.snapshot(key: PropertyKeys.endSnapshot)
    ...
    private static func generateStationDict( _ stations: [SurveyStation]) -> [SurveyStation.Identifier: simd_float3] {
...
extension ScanFile {
    ...
    struct Decode {
        ...
        init(coder: NSCoder) {
            decoder = coder
            version =
                decoder.containsValue(forKey: PropertyKeys.version) ? decoder.decodeInt32(forKey: PropertyKeys.version) : 1
        }
